...
## 十三、纵向比例的重大纠正 + 浓度固定（2026-09-25 晚）

### 1. 纵向补偿系数 1.75 → **1.0**（之前完全搞错了）

**重新标定**：打印 6 张标定标签（默认浓度 + 浓度 1~5），每张含
「500 行竖条」，实测：

| 张 | 500 行竖条长度 | 行距 |
|---|---|---|
| 默认（不发浓度命令） | 43mm | 0.086mm/行 |
| 浓度 1 / 2 / 3 / 4 / 5 | 43mm（**五张完全一致**） | 0.086mm/行 |

→ 纵向行距 ≈ 0.086mm/行 ≈ **300dpi**，与横向 11.9 点/mm 基本一致
→ **纵向不需要任何补偿**（系数 1.0），且**打印浓度不影响行距**

**为什么之前会得出 1.75**：早期用「双块测试」「she.jpg」等样张测得纵向只有
0.047mm/行，据此加了 1.75 倍预拉伸。现在看，那些样张的长度偏短
**不是"行距被压缩"，而是打印过程中丢了行**（见下第 3 点）。用错误的样张
标定出的 1.75，后果是**文字与图片被纵向拉长 1.75 倍**——用户看到
"文本宽高比变了" 就是这个原因。已改正，用户确认文本比例恢复 ✅

### 2. 打印浓度固定为中等（3）

用户要求先固定，界面上的浓度选择器已移除；值在
`PrintSettings.fixedDensity = 3`（1 最淡、5 最深；官方默认 1 偏淡）。
命令仍为 `10 FF 10 00 <3>`。

### 3. 发送方式改为「带响应写入」（官方同款）

- 官方 App 用 Android 默认的 `WRITE_TYPE_DEFAULT`（= 带响应）
- 我们原先用 **withoutResponse**：写快了本地/打印机缓冲溢出会**静默丢包**，
  表现为图案变短、缺底部边距（长任务更明显）
- 现改为 `.withResponse`，每片等 `didWriteValueFor` 再发下一片，由打印机
  应答天然节流。实测 168KB 用 14.4s（≈11.4KB/s，比原限速更快），且必达
- 特征属性确认：`['notify', 'write', 'write-without-response']` 两者都支持

### 4. 已知打印机限制：大面积纯黑长内容会被"丢行"

满宽纯黑长条（100% 黑）实测：1000 行只打出 55~59mm（应 86mm）、
2000 行只打出 57mm（应 172mm）；换成窄条（7% 黑）同样 500 行则精确
打出 43mm。说明打印机对**高能耗内容**会限流并跳过行（加热/热保护），
与发送速率无关（15ms / 25ms / 40ms / withResponse 都一样）。

→ 正常内容（文字、二维码、照片）黑度远低于此，实测一直正常，不受影响。
→ 不建议用满宽纯黑长条做测试（既费纸又触发该限制）。

## 十四、浓度 ↔ 走纸速度 的耦合（决定性发现，2026-09-25 夜）

**现象**：用户报告"选择浓度后打印文本，文本宽高比变了"（文字被纵向压扁）。

**实验**：同一文本任务（App 引擎生成，图案 377 行、无任何纵向拉伸）
分别用浓度 3 与浓度 1 打印：
...
**结论**：打印浓度**同时改变走纸速度**——浓度越高，每行加热时间越长，
走纸越慢，位图的每行走纸距离变小 → 图案被纵向压扁。

**为什么早期标定没发现**：那次标定用的是**窄条**（40 点宽 ≈ 7% 黑度），
能量极低，所以浓度 1~5 都测出同样的 0.086mm/行（六张全 43mm）；
而**文字**（中等黑度）与**照片**（高黑度）会明显受浓度影响。
这也解释了历史上所有矛盾测量：
- 窄条/浅内容 → 0.086mm/行（≈300dpi，无压扁）
- 文字（浓度 3）→ 明显压扁
- 满宽纯黑长条 → 压得最狠（0.055mm/行 甚至更小）

**当前方案**（用户确认）：**浓度固定为 1**（`PrintSettings.fixedDensity = 1`），
纵向系数保持 1.0 —— 文字比例正常，代价是墨色偏淡。
若将来要更深的墨色：需要按该浓度重新标定纵向预拉伸系数
（即在 `rasterize` 里按浓度补偿，而不是把系数写死 1.0）。

**发送方式**：此前已改为「带响应写入」（官方同款），避免长任务静默丢包。
